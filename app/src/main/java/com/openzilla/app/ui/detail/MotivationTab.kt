...
fun MotivationTab(
    ...
    Scaffold(
        floatingActionButton = { FloatingActionButton(onClick = { showAddDialog = true }) { Icon(Icons.Filled.Add, contentDescription = stringResource(R.string.motivation_add_reason)) } }
    ) { padding ->
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding).padding(16.dp)) {
            item {
                Card(modifier = Modifier.fillMaxWidth()) {
                    Column(Modifier.padding(16.dp)) {
                        Text(stringResource(R.string.motivation_quote_title), style = MaterialTheme.typography.titleMedium)
            ...
            item {
                Text(stringResource(R.string.motivation_your_reasons), style = MaterialTheme.typography.titleMedium, modifier = Modifier.padding(top = 20.dp, bottom = 8.dp))
            }
            if (reasons.isEmpty()) {
                item { Text(stringResource(R.string.motivation_no_reasons), color = MaterialTheme.colorScheme.onSurfaceVariant) }
            } else {
                items(reasons, key = { it.id }) { reason ->
                    Card(modifier = Modifier.fillMaxWidth().padding(vertical = 4.dp)) {
                        Row(Modifier.padding(12.dp).fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween, verticalAlignment = Alignment.CenterVertically) {
                            Text(reason.text, modifier = Modifier.weight(1f))
                            IconButton(onClick = { onDeleteReason(reason) }) { Icon(Icons.Filled.Delete, contentDescription = stringResource(R.string.motivation_delete_reason)) }
                        }
                    }
                }
            }
...
private fun SavingsCard(habit: HabitEntity, currencySymbol: String) {
    ...
    Card(modifier = Modifier.fillMaxWidth().padding(top = 12.dp)) {
        Column(Modifier.padding(16.dp)) {
            Text(stringResource(R.string.savings_title), style = MaterialTheme.typography.titleMedium)
            val amount = habit.weeklyAmount
            when {
                habit.costType == HabitCostType.MONEY && amount != null && amount > 0 -> {
                    val saved = amount / 7.0 * elapsedDays
                    Text(stringResource(R.string.savings_money, currencySymbol + "%.2f".format(saved)), modifier = Modifier.padding(top = 8.dp))
                }
                habit.costType == HabitCostType.TIME && amount != null && amount > 0 -> {
                    val savedHours = amount / 7.0 * elapsedDays
                    Text(stringResource(R.string.savings_time, "%.1f".format(savedHours)), modifier = Modifier.padding(top = 8.dp))
                }
                else -> {
                    Text(stringResource(R.string.savings_days, "%.1f".format(elapsedDays)), modifier = Modifier.padding(top = 8.dp))
                }
            }
        }
    }
...
private fun AddReasonDialog(onDismiss: () -> Unit, onConfirm: (String) -> Unit) {
    ...
    androidx.compose.material3.AlertDialog(
        ...
        title = { Text(stringResource(R.string.motivation_new_reason)) },
        text = {
            OutlinedTextField(value = text, onValueChange = { text = it }, label = { Text(stringResource(R.string.motivation_why)) }, modifier = Modifier.fillMaxWidth())
        },
